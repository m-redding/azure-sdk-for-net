...
namespace Azure.Core.Expressions.DataFactory.Tests
{
    public class DataFactoryElementWithConverterTests
    {
        ...
        public void BinaryDataCanHandleNonObjectsValues()
        {
            ...
            Assert.That(GetSerializedStringWithConverter<BinaryData>(dfeString), Is.EqualTo(StringJson));

            var dfeBool = JsonSerializer.Deserialize<DataFactoryElement<BinaryData>>(BoolJson)!;
            Assert.That(GetSerializedStringWithConverter<BinaryData>(dfeBool), Is.EqualTo(BoolJson));

            var dfeInt = JsonSerializer.Deserialize<DataFactoryElement<BinaryData>>(IntJson)!;
            Assert.That(GetSerializedStringWithConverter<BinaryData>(dfeInt), Is.EqualTo(IntJson));
        }
        ...
        public void DeserializationOfListOfT()
        {
            ...
            Assert.That(dfe.ToString(), Is.EqualTo("System.Collections.Generic.List`1[Azure.Core.Expressions.DataFactory.Tests.DataFactoryElementWithConverterTests+TestModel]"));
        }
        ...
        public void FailsIfCanConvertIsFalse()
        {
            ...
            Assert.That(exception!.Message.StartsWith("The converter specified on"), Is.True);
        }
