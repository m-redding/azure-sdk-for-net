...
public class ClientLoggingPolicy : PipelinePolicy
{
    ...
    public ClientLoggingPolicy(ILogger logger, ClientDiagnosticsOptions options)
    {
        _sanitizer = new HttpMessageSanitizer(options.LoggedQueryParameters.ToArray(), options.LoggedHeaderNames.ToArray());
        _logContent = options.IsLoggingContentEnabled;
        _maxLength = options.LoggedContentSizeLimit;
        _assemblyName = options.GetType().Assembly!.GetName().Name!;
        _clientRequestIdHeaderName = options.ClientRequestIdHeaderName;
        _logger = logger;
    }
    ...
    private async ValueTask ProcessSyncOrAsync(PipelineMessage message, IReadOnlyList<PipelinePolicy> pipeline, int currentIndex, bool async)
    {
        ...
        _logger.BeginScope("RequestId: {RequestId}", requestId);
